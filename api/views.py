...
from api.serializer import MyTokenObtainPairSerializer, RegisterSerializer , MarkerSerializer , EventSerializer , ImageSerializer , CommentSerializer , ImageEventSerializer
from rest_framework_simplejwt.views import TokenObtainPairView
from rest_framework import generics
from django.contrib.auth.models import User
from rest_framework.permissions import AllowAny, IsAuthenticated
from rest_framework.decorators import api_view, permission_classes
from api.models import Marker , Event , Image , Comment , ImageEvent
...
class MarkerListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        queryset = Marker.objects.filter(permissionmarker__isnull=False).distinct()
        return queryset
...
class ImageEventUploadView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Save the image data to the database
        image = ImageEvent.objects.create(event_id=event_id, link=f'{link_info.get_link()}/preview')
        serializer = self.get_serializer(image)
